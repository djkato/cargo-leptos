...
pub async fn extra_libs(
    ...
    tokio::spawn(async move {
        ...
        fs::create_dir_all(&proj.site.root_relative_pkg_dir().join("extra")).await?;

        let (envs, line, process) = extra_libs_cargo_process("build", true, &proj)?;
...
pub fn extra_libs_cargo_process(
    cmd: &str,
    wasm: bool,
    proj: &Project,
) -> Result<(String, String, Child)> {
    let mut command = Command::new("cargo");
    let (envs, line) = build_cargo_extra_libs_cmd(cmd, wasm, proj, &mut command);
    Ok((envs, line, command.spawn()?))
}

pub fn build_cargo_extra_libs_cmd(
    cmd: &str,
    wasm: bool,
    proj: &Project,
    command: &mut Command,
    ...
    let mut args = vec![
        ...
        format!(
            ...
            itertools::Itertools::intersperse(
                proj.extra_libs.iter().map(|l| l.name.clone()),
                ",".to_string()
            )
            .collect::<String>()
        ),
        "--lib".to_string(),
        format!("--target-dir={}", &proj.lib.front_target_path.join("extra")),
    ];
